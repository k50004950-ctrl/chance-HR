...
import { sendPushToUser, sendPushToOwner } from '../services/webPush.js';
...
router.post('/check-in', authenticate, validateCheckIn, async (req, res) => {
  try {
    ...
    if (existingRecord && existingRecord.check_in_time) {
      return res.status(400).json({ success: false, message: '이미 출근 체크하셨습니다.' });
    }
...
router.post('/check-out', authenticate, async (req, res) => {
  try {
    ...
    if (existingRecord.check_out_time) {
      return res.status(400).json({ success: false, message: '이미 퇴근 체크하셨습니다.' });
    }
...
router.post('/qr/check', authenticate, async (req, res) => {
  try {
    ...
    if (!token) {
      return res.status(400).json({ success: false, message: 'QR 토큰이 필요합니다.' });
    }

    if (req.user.role !== 'employee') {
      return res.status(403).json({ success: false, message: '근로자 계정만 이용 가능합니다.' });
    }
    ...
    if (!workplace) {
      return res.status(400).json({ success: false, message: '유효하지 않은 QR 코드입니다.' });
    }
    ...
    sendPushToOwner(workplaceId, {
      ...
      return res.json({ success: true, message: 'QR 출근이 기록되었습니다.', checkInTime: now });
    }
...
router.put('/:id', authenticate, async (req, res) => {
  try {
    ...
    if (!record) {
      return res.status(404).json({ success: false, message: '출퇴근 기록을 찾을 수 없습니다.' });
    }
